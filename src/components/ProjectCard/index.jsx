...
export function ProjectCard({ projects, title = '', viewToggle = false, addProject = false }) {
        ...
        <div className={styles.container}>
            ...
            <div className={layout === 'grid' ? styles.projects_grid : styles.projects_list}>
                {projects.map((project) => (
                    <div key={project.id} className={`${layout === 'grid' ? styles.project_card : styles.project_list_item} ${project.seloESG && 'esg'}`}>
                        ...
                        <div className={`${styles.project_info}  ${layout === "list" && styles.list_info}`}>
                            ...
                            <div className={styles.project_meta}>
                                ...
                                <span> • {formataData(project.endDate) ?? ''} </span>
                            </div>
                            <div className={styles.project_footer}>
                                <div className={styles.stats}>
                                    <div className={styles.stats_actions}>
                                        ...
                                        <span>{formatNumberShort(project.stats?.views, 'BR')}</span>
                                    </div>
                                    <div className={styles.stats_actions}>
                                        <LikeButton initialLiked={false} onChange={next => handleLike(projeto.id, next)} />
                                        <span>{formatNumberShort(project.stats?.likes, 'BR')}</span>
                                    </div>
                                </div>
                                <Link href={`/project/${project.id}`} className={`primary_button ${layout === 'list' && styles.cta}`}>
                                    Ver projeto
                                </Link>
                            </div>
                        </div>
